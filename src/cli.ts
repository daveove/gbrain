...
async function handleCliOnly(command: string, args: string[]) {
  ...
  try {
    switch (command) {
      ...
      case 'graph': {
        const { runGraphUsefulness } = await import('./commands/graph-usefulness.ts');
